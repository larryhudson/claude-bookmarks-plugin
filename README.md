...
Type a bookmark using either form:

```
/bookmark remember to investigate the auth regression
bm: remember to investigate the auth regression
```
...
- **`hooks/bookmark.sh`** — fires on `UserPromptSubmit`. If the prompt starts with `/bookmark`, `/bookmarks:bookmark`, or `bm:`, returns `decision: "block"` with a sentinel marker `[bookmark-plugin:v1]` in the reason. The assistant never sees the prompt.
